...
/* Retour affiche à la fin de la bande-annonce YouTube. */
...
export default{async fetch(request,env){const url=new URL(request.url);if(url.pathname==="/api/video")return handleVideo(request);const response=await env.ASSETS.fetch(request);return injectKungFuPanda(request,response)}};
